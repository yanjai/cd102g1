...
try{
	...
	if(isset($_REQUEST["coachNo2"])){
		...
		$Upmember = $Upmember = $_REQUEST["memNo"];
		$update = date("Y/m/d");
		$sql3="INSERT INTO coachcomment(commentNo,coachNo,memNo,comment,score,report,verify,commentdate)values(null,'$UpcoachNo','$Upmember','$Upcontent','$Upnewscore','0','0','$update')";
		$pdo->exec($sql3);
		$coachNo = $_REQUEST["coachNo2"];
	}else{
		$coachNo = $_REQUEST["coachNo"];
	}
    $sql="select * from coach c join coachcomment m on c.coachNo = m.coachNo left outer join member mb on m.memNo = mb.memNo where c.coachNo =  $coachNo and m.verify = 0 ORDER BY m.commentNo desc";
    ...
    <input type="hidden" value='<?php echo $count ?>' id='count'>
			<ul>
				...
				<li>
					...
					<div class="chPerDate"><p><?php echo $item['commentdate']; ?></p></div>
				</li>
                <?php } ?>
			</ul>
		</div>
