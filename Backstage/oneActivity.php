...
<div class="contentArea">
	<div class="actOnlyBox">
		...
		<div class="actRightContent">
			...
			<p>潛水類型: <?php echo $ActRow['actType']; ?></p>
			<!-- <p>適合對象:大</p> -->
			<p>活動地點: <?php echo $ActRow['actLoc']; ?></p>
			<p>活動費用: <?php echo $ActRow['actPrice']; ?></p>
			<h5>活動介紹: </h5>
	...
	<div class="actCoachImgBox">
		...
		<div class="actCoachImg">
			<a href="coach.php#trigger_0<?php echo $actCoachRow['coachNo']?>">  
	...
	<div class="actLightBox actLightBox2">
		...
		<div class="actCoachCd">
				<form id="chCalendar" name="chCalendar">
						...
						<div id="chBtnWrap">
						 <input id="chSubmit" type="submit" value='確認送出'>
						</div>
					   </form>
		</div>
...
<script>
	...
	function refreshDate(Ddata,Gdata){
		...
		for(var p in newData){
			...
			for(var b =0;b<$('.chWhite').length;b++){
				...
				if(oldDate==NewChDate){
					$('.chWhite').eq(b).addClass('chLightgrey');
		...
		for(var q in newGData){
			...
			for(var c =0;c<$('.chLightgrey').length;c++){
				oldGDate = $('.chLightgrey').eq(c).find('input').val();
				console.log(oldGDate);
				if(oldGDate==NewGata){
					$('.chLightgrey').eq(c).addClass('chWhite');
					$('.chLightgrey').eq(c).removeClass('chLightgrey');
				}
			}
		}
