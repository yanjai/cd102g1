...
<head>
	...
	*{
	...
	.detialTable .b_gearName{
		width:30%;
	}
	.detialTable .b_table_img{
		text-align:center;
		/* display:block; */
		/* margin:auto; */
		width:20%;
	...
	$.ajax({
		...
		success: function(data){
            ...
            $('.detialBtn').click(function(){
                $('.detialBox').css('display','block');
                var gearNo = $(this).prev().val();
                $.ajax({
                    url: 'memGearOrderDetial.php',	
                    dataType: 'text',
                    type: 'POST',		
                    data: {gearNo:gearNo},	
                    success: function(data){
                        $('.detialInner').html(data);
                      
                        $('.detialBtn').click(function(){
                            $('.detialBox').css('display','block');
                        })
						$('.detiaClose').click(function(){
							$('.detialBox').css('display','none');
						})
                    }
                });
            })
