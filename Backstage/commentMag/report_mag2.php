<?php 
    try{
        ...
        if(isset($_REQUEST['commentNo'])){
            ...
            $report = 0;
            if($verifyVal==0){
                $sql2 = "UPDATE  activitycomment set report = :report, verify = :verify where commentNo = :commentNo";
                $upcoach = $pdo->prepare($sql2);
                $upcoach->bindParam(':report', $report);
                $upcoach->bindParam(':verify', $verifyVal);
            }else{
                $sql2 = "UPDATE  activitycomment set verify = :verify where commentNo = :commentNo";
                $upcoach = $pdo->prepare($sql2);
                $upcoach->bindParam(':verify', $verifyVal);
            }
        ...
        $sql = "select * from activitycomment where report > 0";
        $bot = $pdo->query($sql);
